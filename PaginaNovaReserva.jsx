...
import { calcularValorTotal, criarReserva, obterUsuarioAtual, preverPreco } from '../../services/api'
...
export default function PaginaNovaReserva({ hotel, quarto, aoNavegar }) {
  ...
  const [precoBackend, setPrecoBackend] = useState(null)
  ...
  useEffect(() => {
    if (!form.data_checkin || !form.data_checkout) return
    preverPreco({ quarto_id: quarto.id, ...form }).then(setPrecoBackend).catch(() => setPrecoBackend(null))
  }, [quarto.id, form])
  ...
  if (sucesso) {
    return (
      <div className="alert alert-success shadow-sm p-4">
        <h4 className="alert-heading fw-bold">Reserva enviada para processamento!</h4>
        <p className="mb-1">
        ...
        <p className="mb-2 fw-bold">Valor estimado: R$ {Number(sucesso.valor_total ?? valorTotal).toFixed(2)}</p>
        <p className="mb-3">Status inicial: <strong>{sucesso.status}</strong>. A reserva será processada pelo worker.</p>
        <button
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="d-flex justify-content-between align-items-center">
              <div>
                ...
                <p className="fs-4 fw-bold text-primary mb-0">Total: R$ {Number(precoBackend?.total ?? valorTotal).toFixed(2)}</p>
              </div>
